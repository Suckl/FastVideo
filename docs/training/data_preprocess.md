...
selection, manifest content, preprocessing batch size, worker/world sharding,
component precision, and tokenizer sequence length. Each item also records a
fingerprint over its cleaned caption, media bytes, and pinned Stage 4
duration/FPS/resolution contract. Decoder-probed resolution must match the
manifest and does not silently replace manifest values during bucket planning.
